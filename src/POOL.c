/*
 * pool.asm - --- Jemm's memory pool implementation --- originally written by Michael Devore --- Public Domain --- to be assembled with JWasm or Masm v6.1+ (C17 standard)
 *
 * Architectural Role:
...
// .486P
// .model FLAT
void Pool_GetPhysAddr(void)
{
    /*
     * Mapped logic from Pool_GetPhysAddr in POOL.ASM:
...
void Pool_GetFree4KPages(void)
{
    /*
     * Mapped logic from Pool_GetFree4KPages in POOL.ASM:
     ...
     * 
     *     @dprintf ?POOLDBG,<"Pool_GetFree4KPages: %X, dwUsedMem4K=%X, dwMaxMem4K=%X",10>, eax, dwUsedMem4K, dwMaxMem4K
     *     ret
     *     align 4
     * 
...
void Pool_GetFree16KPages(void)
{
    /*
     * Mapped logic from Pool_GetFree16KPages in POOL.ASM:
     ...
     *     add eax, esi
     * 
     *     @dprintf ?POOLDBG, <"Pool_GetFree16KPages=%X max4k=%X total4k=%X XMS-4k=%X",10>, eax, [dwMaxMem4K], [dwTotalMem4K], edx
     *     pop edx
     *     pop ecx
     *     pop esi
     *     ret
...
void Pool_ExpandBlock(void)
{
    /*
     * Mapped logic from Pool_ExpandBlock in POOL.ASM:
     * Pool_ExpandBlock PROC
     * 
     *     pushad
     * 
     *     @dprintf ?POOLDBG, <"Pool_ExpandBlock: esi=%X, addrK=%X, pArray=%X",10>, esi, [esi].POOL_SYSTEM_INFO.psi_addressK, [esi].POOL_SYSTEM_INFO.psi_descptr
     * 
...
void Pool_ExpandAnyBlock(void)
{
    /*
     * Mapped logic from Pool_ExpandAnyBlock in POOL.ASM:
...
void Pool_Allocate4KPage(void)
{
    /*
     * Mapped logic from Pool_Allocate4KPage in POOL.ASM:
...
void Pool_Allocate16KPage(void)
{
    /*
     * Mapped logic from Pool_Allocate16KPage in POOL.ASM:
...
void Pool_FindBlock(void)
{
    /*
     * Mapped logic from Pool_FindBlock in POOL.ASM:
     * Pool_FindBlock proc
     * 
     *     mov esi, [LastBlockFreed]
     * 
...
void Pool_Free4KPage(void)
{
    /*
     * Mapped logic from Pool_Free4KPage in POOL.ASM:
...
void Pool_Free16KPage(void)
{
    /*
     * Mapped logic from Pool_Free16KPage in POOL.ASM:
     ...
     * @@fail1:
     * if ?POOLDBG
     *     @dprintf ?POOLDBG, <"Pool_Free16kPage failed edx=%X">, edx
     *     sub edx, [EMSPageAllocationStart]
     *     shr edx, 2
     *     @dprintf ?POOLDBG, <" EMS page=%X",10>, edx
     *     jmp @@fail
     * endif
     * @@fail2:
...
void Pool_GetUnusedBlock(void)
{
    /*
     * Mapped logic from Pool_GetUnusedBlock in POOL.ASM:
...
void Pool_PrepareBlock(void)
{
    /*
     * Mapped logic from Pool_PrepareBlock in POOL.ASM:
...
void Pool_FreeAllBlocks(void)
{
    /*
     * Mapped logic from Pool_FreeAllBlocks in POOL.ASM:
...
void Pool_TryFreeToXMS(void)
{
    /*
     * Mapped logic from Pool_TryFreeToXMS in POOL.ASM:
...
void Pool_AllocBlocksForEMB(void)
{
    /*
     * Mapped logic from Pool_AllocBlocksForEMB in POOL.ASM:
...
void Pool_AllocateEMBForPool(void)
{
    /*
     * Mapped logic from Pool_AllocateEMBForPool in POOL.ASM:
...
void Pool_GetFreeXMSPages(void)
{
    /*
     * Mapped logic from Pool_GetFreeXMSPages in POOL.ASM:
...
void Pool_FreeEMB(void)
{
    /*
     * Mapped logic from Pool_FreeEMB in POOL.ASM:
     * Pool_FreeEMB PROC
     * 
     *     pushad
     * 
...
// if ?POOLDBG
void CheckBlockIntegrity(void)
{
    /*
     * Mapped logic from CheckBlockIntegrity in POOL.ASM:
...
void Pool_Init1(void)
{
    /*
     * Mapped logic from Pool_Init1 in POOL.ASM:
...
void Pool_Init2(void)
{
    /*
     * Mapped logic from Pool_Init2 in POOL.ASM:
     ...
     *     je @@setblocks
     * 
     *     @dprintf ?INITDBG, <"pool sharing is off, 4k pages remaining=%X dwMaxMem4k=%X",10>, eax, dwMaxMem4K
     * 
