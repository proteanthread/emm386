/*
 * jclock2.asm - this is a modified JCLOCK version which uses the Jemm API to hook int 1Ch. Thus, it can be unloaded. (C) 2007 Alexey Voskov Compile with FASM for WIN32 and change PE signature into PX (C17 standard)
 *
 * Architectural Role:
 *   Serves as the C counterpart source representing JCLOCK2.ASM.
 *
