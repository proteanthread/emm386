/*
 * setargv.inc - --- read the commandline at PSP:81h --- and create an argc/argv structure on the stack. --- in: ES=PSP, DS=DGROUP, SS=DGROUP --- out: _argc (=[bp-2]) --- _argv (=[bp-4]) --- all std registers modified (including SP) (C17 standard)
 *
 * Architectural Role:
 *   Serves as the C counterpart header representing SETARGV.INC.
 *
...
static inline void _setargv(void)
{
    /*
     * Mapped logic from _setargv in SETARGV.INC:
     * _setargv proc
     * 
     *     mov bp, sp
