/*
 * fboot.asm - --- Set /D and /P option for currently loaded fastboot JLM. --- This program uses the API installed by FASTBOOT.DLL; --- alternatively, one can unload & reload FASTBOOT.DLL. (C17 standard)
 *
 * Architectural Role:
 *   Serves as the C counterpart source representing FBOOT.ASM.
 *
...
{
    /*
     * Mapped logic from getoption in FBOOT.ASM:
     * getoption proc
     * 	cld
     * 	mov si,81h
     * nextchr:
...
{
    /*
     * Mapped logic from main in FBOOT.ASM:
     * main proc
     * 
     * local dwFB:dword
     * 
...
{
    /*
     * Mapped logic from lfout in FBOOT.ASM:
     * lfout proc
     * 	mov dx,DStr(13,10)
     * 	mov ah,9
     * 	int 21h
