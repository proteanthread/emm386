/*
 * jlstub.asm - --- MZ stub to run jload.exe --- to be assembled with Masm or JWasm --- it's a derivate of dpmildxx... (C17 standard)
 *
 * Architectural Role:
 *   Serves as the C counterpart source representing JLSTUB.ASM.
 *
...
#define szPgm [bp+00h] // execute program name ("JLOAD.EXE")
#define szParm [bp-MAXPATH] // application name (from environment)

void launch(void)
{
    /*
     * Mapped logic from launch in JLSTUB.ASM:
     * launch proc
     * 
     * 	xor DX,DX
     * 	mov cl,20h
...
{
    /*
     * Mapped logic from start in JLSTUB.ASM:
     * start proc
     * 
...
{
    /*
     * Mapped logic from GetAppName in JLSTUB.ASM:
     * GetAppName proc
     * 	SUB DI,DI
     * 	mov al,00
     * 	mov cx,-1
...
{
    /*
     * Mapped logic from SearchPath in JLSTUB.ASM:
     * SearchPath proc
     * 	SUB DI,DI
     * nextitem:
...
void SearchJLoad(void)
{
    /*
     * Mapped logic from SearchJLoad in JLSTUB.ASM:
     * SearchJLoad proc
     * 	lea DI,szPgm
     * 	mov dx,di
     * nextentry:					;<----
...
// szPath  db   'PATH='
#define SIZPATH $ - szPath

// dNotFnd db "Can't find "
// ldrname	label byte
// db 'JLOAD.EXE'
#define SIZLDRNAME $ - ldrname
// db '$'
