/*
 * ems5b.asm - *** test EMS 5B --- 5b00: get alternate map register set ( if BL=0 on ret, es:di will be current pointer to context save area ) --- 5b01: set alternate map register set in BL --- if BL=0, es:di will be current context save area --- 5b02: get size of alternate map register set in dx (C17 standard)
 *
 * Architectural Role:
 *   Serves as the C counterpart source representing EMS5B.ASM.
 *
...
{
    /*
     * Mapped logic from memset in EMS5B.ASM:
...
{
    /*
     * Mapped logic from main in EMS5B.ASM:
     * main proc c
     * 
     * 	mov ax,3567h
     * 	int 21h
