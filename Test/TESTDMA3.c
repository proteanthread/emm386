/*
 * testdma3.asm - --- test FD read in UMB --- using Int 13h --- this variant ensures that a 64kb border is crossed, --- forcing the EMM to either "auto remap" PTEs (EMM386) or --- use the DMA buffer (Jemm). (C17 standard)
 *
 * Architectural Role:
 *   Serves as the C counterpart source representing TESTDMA3.ASM.
 *
...
{
    /*
     * Mapped logic from writefile in TESTDMA3.ASM:
...
{
    /*
     * Mapped logic from main in TESTDMA3.ASM:
     * main proc c
     * 
     * local stat1:word
     * local stat2:word
