/*
 * xmstest5.asm - --- XMSTEST5: test block resize (shrink) --- this variant allocates a 64MB block with xms v3, --- but tries to resize if with xms v2 to 32MB. --- Public Domain. --- to be assembled with JWasm or Masm v6. (C17 standard)
 *
 * Architectural Role:
 *   Serves as the C counterpart source representing XMSTEST5.ASM.
 *
...
{
    /*
     * Mapped logic from testproc in XMSTEST5.ASM:
     * testproc proc
     * 
     * local handle:word
     * local wRC:word
...
{
    /*
     * Mapped logic from main in XMSTEST5.ASM:
     * main    proc c
     * 
     * 	mov ax,4300h
     * 	int 2fh
...
{
    /*
     * Mapped logic from start in XMSTEST5.ASM:
     * start   proc
     * 
     * 	mov ax,@data
     * 	mov ds,ax
