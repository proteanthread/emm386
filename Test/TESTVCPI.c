/*
 * testvcpi.asm - --- the purpose of the test is to see if page table 0, returned by int 67h, ax=DE01h --- is updated by the VCPI host. That's claimed by the VCPI docs, but of course it isn't. --- It's simply not feasible, because the host doesn't know if the table is still "valid". --- if segments are defined BEFORE .model, --- the alignment can be set differently (JWasm only). (C17 standard)
 *
 * Architectural Role:
 *   Serves as the C counterpart source representing TESTVCPI.ASM.
 *
...
#define ?DISPPTE 0 // 1=display PTE entries before/after mapping change.
#define ?ENTRIES 0F0h // PTE entries to display
...
{
    /*
     * Mapped logic from println in TESTVCPI.ASM:
     * println proc
...
{
    /*
     * Mapped logic from defint in TESTVCPI.ASM:
     * defint proc near
     * 	push eax
     * 	mov al,0Bh
     * 	out 0A0h,al
...
{
    /*
     * Mapped logic from int31 in TESTVCPI.ASM:
     * int31 proc
     * 	cmp ax,300h
     * 	jz is300
     * 	jmp exit
...
{
    /*
     * Mapped logic from int21 in TESTVCPI.ASM:
     * int21 proc
     * 
     *     push es
     *     push di
...
{
    /*
     * Mapped logic from DispPTEs in TESTVCPI.ASM:
...
{
    /*
     * Mapped logic from setdescriptors in TESTVCPI.ASM:
     * setdescriptors proc
     * 	mov ax,cs
     * 	movzx eax,ax
     * 	shl eax, 4
...
{
    /*
     * Mapped logic from rm2pm in TESTVCPI.ASM:
     * rm2pm proc
     * 	mov pmtr.attrib,89h
     * 	cli
     * 	pushf
...
{
    /*
     * Mapped logic from pm2rm in TESTVCPI.ASM:
     * pm2rm proc
     * 
     * 	movzx edx,sp
     * 	movzx eax, [segds]
...
{
    /*
     * Mapped logic from pmgo in TESTVCPI.ASM:
     * pmgo proc
     * 
     * 	cli
     * 
...
{
    /*
     * Mapped logic from runvcpi in TESTVCPI.ASM:
     * runvcpi proc near
     * 
...
{
    /*
     * Mapped logic from getparm in TESTVCPI.ASM:
     * getparm proc near
     * 	ret
     * getparm endp
     */
...
{
    /*
     * Mapped logic from main in TESTVCPI.ASM:
     * main proc c
     * 
     * 	mov di, offset ptab
