/*
 * movexbda.asm - --- MOVEXBDA moves XBDA to low memory. If the XBDA is already moved --- or if it is too large, nothing is done. --- MOVEXBDA hooks interrupt 19h and will restore the XBDA to its --- previous location if this interrupt is called. --- MOVEXBDA is Public Domain. (C17 standard)
 *
 * Architectural Role:
 *   Serves as the C counterpart source representing MOVEXBDA.ASM.
 *
...
#define XBDAADR 000Eh // address of XBDA segment in BIOS data region
#define MEMSIZE 0013h // size of memory in kB in BIOS data region
#define ?WELCOME 0 // display message at start
...
{
    /*
     * Mapped logic from devstrat in MOVEXBDA.ASM:
     * devstrat proc far
...
{
    /*
     * Mapped logic from devint in MOVEXBDA.ASM:
     * devint proc far
     * 	push bx
     * 	push ds
     * 	lds bx,cs:[cmdptr]
...
{
    /*
     * Mapped logic from myint19 in MOVEXBDA.ASM:
     * myint19 proc
     * 	push ds
     * 	push 40h
     * 	pop ds
...
void devintfirst(void)
{
    /*
     * Mapped logic from devintfirst in MOVEXBDA.ASM:
