/*
 * umbm.asm - *** UMBM is a DOS device driver, but will not install permanently: *** if UMBs activated by UMBPCI are found, UMBM will check if *** a XMS host is present. If yes, it will hook into the XMS chain *** and add support for UMBs there. If no, it will install a micro *** XMS host which just provides UMBs. *** During the boot process, if line DOS=UMB is present in CONFIG.SYS, ** DOS will query for UMBs every time a driver has been loaded. If UMBs *** are provided, DOS will grab them. So UMBM's lifetime usually should be *** very short. Once the UMBs are grabbed, UMBM will remove itself from *** the XMS/Int 2Fh chain. *** To not use any DOS memory, UMBM copies itself into the first UMB. *** This assumes that DOS first allocates all available UMBs *before* *** "using" them. All DOSes I know do so, but AFAIK this is not documented *** and therefore a small risk remains. (C17 standard)
 *
 * Architectural Role:
 *   Serves as the C counterpart source representing UMBM.ASM.
 *
...
#define ?MOVEINUMB 1 // move in 1. umb
#define ?MOVEXBDA 1 // /XBDA option
...
{
    /*
     * Mapped logic from umbrou in UMBM.ASM:
...
{
    /*
     * Mapped logic from devstrat in UMBM.ASM:
     * devstrat proc far
...
{
    /*
     * Mapped logic from devint in UMBM.ASM:
     * devint proc far
     * 	pusha
     * 	push ds
     * 	push es
...
{
    /*
     * Mapped logic from ignname in UMBM.ASM:
...
{
    /*
     * Mapped logic from _strout in UMBM.ASM:
...
{
    /*
     * Mapped logic from _wordout in UMBM.ASM:
     * _wordout proc
     * 
     * 	push ax
     * 	mov al,ah
...
{
    /*
     * Mapped logic from getpar in UMBM.ASM:
     * getpar proc
     * nextitem:
     * 	mov al,es:[si]
     * 	cmp al,cr
...
{
    /*
     * Mapped logic from makecaps in UMBM.ASM:
     * makecaps proc
     * 	cmp al,'a'
     * 	jb iscaps
     * 	cmp al,'z'
...
{
    /*
     * Mapped logic from ishex in UMBM.ASM:
     * ishex proc
     * 	cmp al,'0'
     * 	jb @@no
     * 	cmp al,'9'
...
{
    /*
     * Mapped logic from gethexnumber in UMBM.ASM:
     * gethexnumber proc
     * 	push dx
     * 	mov ch,00
     * 	mov dx,0000
...
{
    /*
     * Mapped logic from getregion in UMBM.ASM:
...
void searchumbpciblocks(void)
{
    /*
     * Mapped logic from searchumbpciblocks in UMBM.ASM:
...
{
    /*
     * Mapped logic from movexbda in UMBM.ASM:
     * movexbda proc
     * 	push 40h
     * 	pop es
     * 	mov dx,es:[0013h]
...
{
    /*
     * Mapped logic from main in UMBM.ASM:
     * main proc near
     * 
     * 	push cs
     * 	pop ds
...
{
    /*
     * Mapped logic from explain in UMBM.ASM:
     * explain proc
     * 	push cs
     * 	pop ds
...
{
    /*
     * Mapped logic from main_exe in UMBM.ASM:
     * main_exe proc c
     * 
     * 	call explain
     * 	mov ax,4c00h
