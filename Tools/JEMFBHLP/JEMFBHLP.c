/*
 * jemfbhlp.asm - *** save some interrupt vectors the way MS-DOS does *** needed for FreeDOS if Jemm386's FASTBOOT option is used (C17 standard)
 *
 * Architectural Role:
 *   Serves as the C counterpart source representing JEMFBHLP.ASM.
 *
...
{
    /*
     * Mapped logic from devstrat in JEMFBHLP.ASM:
     * devstrat proc far
...
{
    /*
     * Mapped logic from devint in JEMFBHLP.ASM:
     * devint proc far
     * 	push ds
     * 	push bx
     * 	lds bx,cs:[cmdptr]
...
{
    /*
     * Mapped logic from int19 in JEMFBHLP.ASM:
     * int19 proc
     * 	cld
     * 	push 70h
     * 	pop ds
...
#define NUMVECS $ - vecstable

void devintfirst(void)
{
    /*
     * Mapped logic from devintfirst in JEMFBHLP.ASM:
