/*
 * cpuid.asm - --- CPUID displays status of CPU. --- Public Domain. --- Masm syntax. To be assembled with JWasm or Masm. --- uses 16-bit printf (C17 standard)
 *
 * Architectural Role:
 *   Serves as the C counterpart source representing CPUID.ASM.
 *
...
{
    /*
     * Mapped logic from hascpuid in CPUID.ASM:
     * hascpuid proc
     * 	push di
     * 	mov di,sp
...
void print_cpuid_regs(void)
{
    /*
     * Mapped logic from print_cpuid_regs in CPUID.ASM:
...
{
    /*
     * Mapped logic from main in CPUID.ASM:
     * main proc c
     * 
     * 	pushf
     * 	mov ax,7000h
